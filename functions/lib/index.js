...
function sessionCoinPoints(collectionName, data) {
    if (typeof data.pointsEarned === "number")
        return Math.max(0, data.pointsEarned);
    if (collectionName === "sudokuSessions") {
        const points = data.difficulty === "Hard" ? 400 : data.difficulty === "Medium" ? 200 : 100;
        return Math.max(0, points);
    }
    if (collectionName === "connectDotsSessions")
        return Math.max(100 - Math.max(0, Number(data.hintsUsed || 0)) * 25, 0);
...
const scoreToBongoCoins = (score) => Math.max(Math.floor(Math.max(score, 0) / 250), 0);
async function reconcilePlayerCoins(phone, fallbackName = "Player") {
...
function referralCoinsForScore(score) {
    if (!Number.isFinite(score) || score < REFERRAL_MIN_SCORE)
        return 0;
...
async function redeemEligibleReferralForSession(params) {
    ...
    if (!/^07\d{8}$/.test(newUserPhone))
        return { redeemed: false, reason: "invalid-phone" };
...
exports.reconcileAllPlayerCoins = functions.https.onCall(async (data, context) => {
    if (!context.auth)
    ...
    for (const collectionName of Object.values(COIN_LEADERBOARDS)) {
        ...
        snapshot.docs.forEach(entry => { const phone = String(entry.data().phone || entry.id); if (/^0\d{9}$/.test(phone))
            phones.add(phone); });
    }
    for (const config of COIN_SESSION_COLLECTIONS) {
        const snapshot = await db.collection(config.collection).get();
        snapshot.docs.forEach(entry => { const phone = String(entry.data().phone || ""); if (/^0\d{9}$/.test(phone))
            phones.add(phone); });
    }
    const results = [];
    for (const phone of phones)
        results.push(await reconcilePlayerCoins(phone));
    return { success: true, reconciled: results.length };
});
exports.onBongoMarketOrderChanged = functions.firestore
    ...
    for (const phone of phones) {
        if (/^0\d{9}$/.test(phone))
            await reconcilePlayerCoins(phone);
    }
});
function postScoreToSql(msisdn, score) {
...
async function updateQuestProgress(phone, type) {
    try {
...
exports.claimQuestReward = functions.https.onCall(async (data) => {
    const { phone, questId, dateKey } = data;
    if (!phone || !questId)
        throw new functions.https.HttpsError("invalid-argument", "Missing phone or questId");
    ...
    return await db.runTransaction(async (tx) => {
        ...
        if (!progress.completed || !progress.readyToClaim)
            throw new functions.https.HttpsError("failed-precondition", "Quest not completed yet");
        if (progress.claimed)
...
exports.startGameTimer = functions.https.onCall(async (data) => {
    const validGames = ["bongo", "bible", "math", "biology", "generalKnowledge", "sudoku", "connectDots"];
...
function tournamentWeekStartMs(date = new Date()) {
    const local = new Date(date.toLocaleString("en-US", { timeZone: "Africa/Nairobi" }));
    const day = local.getDay() || 7;
...
function tournamentPointBreakdown(input) {
...
async function rebuildCurrentTopScorersTournament() {
    ...
    for (const config of collections) {
        ...
        snap.docs.forEach(docSnap => {
            ...
            if (playedAtMs && playedAtMs < weekStartMs)
                return;
...
exports.rebuildTopScorersTournament = functions.https.onCall(async (_data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Admin sign-in required");
    return rebuildCurrentTopScorersTournament();
});
exports.saveTopScorersTournamentSettings = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Admin sign-in required");
    ...
    const rewards = Array.isArray(data?.rewards) ? data.rewards.slice(0, 6).map((reward) => ({
        rank: String(reward?.rank || "Reward").slice(0, 40),
        title: String(reward?.title || "Reward Pack").slice(0, 60),
        items: Array.isArray(reward?.items) ? reward.items.map((item) => String(item).trim().slice(0, 80)).filter(Boolean).slice(0, 6) : [],
    })) : [];
    const payload = {
        title,
        subtitle,
    ...
    if (typeof data?.endsAt === "string" && data.endsAt) {
        ...
        if (!Number.isNaN(end.getTime()))
            payload.endsAt = admin.firestore.Timestamp.fromDate(end);
    }
    await db.collection("tournamentSettings").doc("topScorers").set(payload, { merge: true });
    return { success: true };
});
function normalizePublicTournamentGame(value) {
    const normalized = String(value || "").toLowerCase().replace(/[\s_-]+/g, "");
    if (normalized === "general" || normalized === "generalknowledge" || normalized === "bongo")
        return "generalKnowledge";
    if (normalized === "sport" || normalized === "sports")
        return "sports";
    if (normalized === "car" || normalized === "cars" || normalized === "carlogos")
        return "carLogos";
    if (normalized === "brand" || normalized === "brands" || normalized === "brandlogos")
        return "brandLogos";
    if (normalized === "trick" || normalized === "trickquestions")
        return "trickQuestions";
    if (normalized === "kenya" || normalized === "kenyatrivia")
        return "kenyaTrivia";
    return "generalKnowledge";
}
function isPublicTournamentGame(value) {
    return value === "generalKnowledge" || value === "sports" || value === "carLogos" || value === "brandLogos" || value === "trickQuestions" || value === "kenyaTrivia";
}
function shuffleDocs(items) {
    const copy = [...items];
...
function sanitizeTournamentRewards(data) {
    return Array.isArray(data?.rewards) ? data.rewards.slice(0, 8).map((reward) => ({
        rank: String(reward?.rank || "Reward").trim().slice(0, 40),
        title: String(reward?.title || "Reward Pack").trim().slice(0, 60),
        items: Array.isArray(reward?.items) ? reward.items.map((item) => String(item).trim().slice(0, 90)).filter(Boolean).slice(0, 8) : [],
    })) : [];
}
function timestampFromInput(value) {
    ...
    if (typeof value !== "string")
        throw new functions.https.HttpsError("invalid-argument", "Invalid tournament date");
    const date = new Date(value);
    if (Number.isNaN(date.getTime()))
        throw new functions.https.HttpsError("invalid-argument", "Invalid tournament date");
    return admin.firestore.Timestamp.fromDate(date);
}
function tournamentIsOpen(data, now = Date.now()) {
...
async function addQuizTournamentPoints(input) {
    if (!isPublicTournamentGame(input.game) || !/^0\d{9}$/.test(input.phone))
        return;
    const breakdown = tournamentPointBreakdown(input);
    if (breakdown.points <= 0)
        return;
    ...
    snap.docs.forEach(tournamentSnap => {
        ...
        if (!tournamentIsOpen(tournament, now))
            return;
...
exports.saveQuizTournament = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Admin sign-in required");
...
const fallbackTournamentQuestionCollections = {
    generalKnowledge: ["generalKnowledgeQuestions", "genQuizQuestions"],
};
function fallbackTournamentQuestion(collectionName, id, data, quizType) {
...
async function loadFallbackTournamentQuestions(quizType) {
    ...
    for (const collectionName of collectionNames) {
        ...
        const rows = snap.docs
            ...
            .filter((question) => Boolean(question && question.active !== false));
        if (rows.length)
            return rows;
    }
    return [];
}
exports.submitQuizTournamentAnswers = functions.https.onCall(async (data) => {
    ...
    if (!tournamentIsOpen(tournament))
        throw new functions.https.HttpsError("failed-precondition", "Tournament is not open");
    if (!isPublicTournamentGame(tournamentQuizType))
        throw new functions.https.HttpsError("invalid-argument", "Unsupported tournament game");
    const entryRef = tournamentRef.collection("entries").doc(phone);
    const existingEntry = await entryRef.get();
    if (existingEntry.exists)
        throw new functions.https.HttpsError("already-exists", "You have already played this tournament");
    const requestedIds = Array.isArray(data?.questionIds) ? data.questionIds.map((id) => String(id)).filter(Boolean).slice(0, 15) : [];
    const isBankQuestion = (q) => q && q.active !== false && Array.isArray(q.options) && normalizePublicTournamentGame(q.quizType) === tournamentQuizType;
    let questions = [];
    // Preferred path: score the exact bank questions the player was shown,
    // fetched directly by id (robust regardless of bank size).
    const bankIds = requestedIds.filter((id) => !id.includes(":"));
    if (bankIds.length) {
        const docs = await db.getAll(...bankIds.map((id) => db.collection(TOURNAMENT_QUESTION_BANK).doc(id)));
        questions = docs
            .filter(d => d.exists)
    ...
    if (!questions.length) {
        ...
        if (!questions.length)
            questions = shuffleDocs(allQuestions).slice(0, 15);
    }
    if (questions.length < 1 || questions.length > 15)
        throw new functions.https.HttpsError("failed-precondition", "Tournament must have 1 to 15 questions");
    let correct = 0;
    ...
    questions.forEach((question) => {
        ...
        if (normalized === Number(question.answer))
            correct += 1;
    });
    ...
    await Promise.all([
        ...
        eventRef.set({ phone, name, game: tournamentQuizType, questionIds: questions.map((question) => question.id), answers: normalizedAnswers, correct, wrong, totalQuestions, score, ...breakdown, createdAt: admin.firestore.FieldValue.serverTimestamp() }),
...
exports.rebuildQuizTournament = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Admin sign-in required");
    const tournamentId = typeof data?.tournamentId === "string" ? data.tournamentId : "";
    if (!tournamentId)
...
// ─────────────────────────────────────────────────────────────────────────────
exports.connectDotsDeposit = functions.https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
    res.set("Access-Control-Allow-Headers", "Content-Type");
    if (req.method === "OPTIONS") {
...
exports.saveConnectDotsScore = functions.https.onCall(async (data) => {
    if (typeof data.name !== "string" || !data.name.trim())
        throw new functions.https.HttpsError("invalid-argument", "Invalid name");
    if (typeof data.phone !== "string" || !/^0\d{9}$/.test(data.phone))
        throw new functions.https.HttpsError("invalid-argument", "Invalid phone");
