...
import {Home, Trophy, Zap, FolderOpen, RotateCw, Clock3, Bell, Gift, ShieldCheck, Users, CalendarCheck, Sparkles, Megaphone, BookOpen, Coins, Trash2, ArrowLeft, PlusCircle, Wallet, LogIn, UserPlus, HelpCircle, UserCog, History, ClipboardList, Share2, Headphones, LogOut} from 'lucide-react';
import {
    FaYoutube,
    FaFacebook,
    FaInstagram,
...
import wheelImg from "../../assets/wheel-hero.webp";
...
import { AppTopBar, type SettingsItem } from "./AppTopBar.tsx";
import {DailyBonusModal} from "./DailyBonusModal.tsx";
...
interface Props {
    ...
    onWallet?: () => void;
    onMarket?: () => void;
    /** "landing" shows the Browse portal; "arena" shows the Bongo Quiz arena hero. */
    variant?: "landing" | "arena";
    /** Called when the featured Bongo Quiz card is played from the Browse portal. */
    onPlayBongo?: () => void;
}
...
export const HomeScreen: FC<Props> = ({
    ...
    const bongoSettingsItems: SettingsItem[] = [
        { key: "htp", label: "How to Play", icon: <HelpCircle size={18} />, onClick: () => setShowHTP(true) },
        { key: "history", label: "Game History", icon: <History size={18} />, onClick: () => onHistory?.() },
        { key: "share", label: "Share", icon: <Share2 size={18} />, onClick: handleInviteFriends },
        { key: "profile", label: "Edit Profile", icon: <UserCog size={18} />, onClick: () => { setAuthMode("login"); setStartAfterAuth(false); setShowNameModal(true); } },
    ];
    ...
    return (
        ...
        <div className="home-root">
            {variant === "arena" && <AppTopBar settingsItems={bongoSettingsItems} />}
            {variant !== "arena" && (
            ...
            <div className="home-content">
                {/* ── Hero section: text + wheel (arena only) ── */}
                {variant === "arena" && (
