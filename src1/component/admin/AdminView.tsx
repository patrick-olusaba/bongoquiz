...
import {AdminSumTen} from "./AdminSumTen.tsx";
...
const TABS: { id: AdminTab; label: string; icon: IconType }[] = [
    ...
    {id: "sumten", label: "SumTen", icon: FaCalculator},
...
function Dashboard({changeTab}: { changeTab: (t: AdminTab) => void }) {
    ...
    useEffect(() => {
        ...
        const firebaseReads = [
            ...
            {label: "SumTen sessions", promise: getDocs(collection(db, "sumTenSessions"))},
            {label: "SumTen payments", promise: getDocs(query(collection(db, "payments"), where("game", "==", "SUMTEN")))},
        ...
        Promise.allSettled(firebaseReads.map(r => r.promise)).then(results => {
            ...
            const [playersR, sessR, payR, lbR,  bqSessR, bqPayR, mqSessR, mqPayR, bioSessR, bioPayR, genSessR, genPayR, sdkSessR, sdkPayR, sdkLegacyPayR, cdSessR, cdPayR, cdLegacyPayR, sumTenSessR, sumTenPayR, dismissedR, auditR, announcementsR] = results;
            const allPayments = snap(payR).map((d: any) => ({_id: d.id, ...d.data()}));
            const knownGames = new Set(["BIBLEQUIZ", "MATHQUIZ", "BIOLOGYQUIZ", "GENERALKNOWLEDGE", "SUDOKU", "CONNECT_DOTS", "SUMTEN"]);
            ...
            const sumTenSessions = snap(sumTenSessR).map((d: any) => d.data());
            const sumTenPayments = snap(sumTenPayR).map((d: any) => d.data());
            ...
            const allPaid = [...bongoPayments, ...biblePayments, ...mathPayments, ...bioPayments, ...genPayments, ...sudokuPayments, ...connectDotsPayments, ...sumTenPayments].filter(p => p.status === "paid");
            const totalRevenue = allPaid.reduce((a, p) => a + (p.amount ?? 0), 0);
            const totalSessions = bongoSessions.length + bibleSessions.length + mathSessions.length + bioSessions.length + genSessions.length + sudokuSessions.length + connectDotsSessions.length + sumTenSessions.length;
            ...
            const allSessions = [
                ...
                ...sumTenSessions.map((x: any) => ({...x, gameName: "SumTen"})),
            ];
            ...
            const gamePerformance = [
                ...
                {name: "SumTen", color: "#f97316", g: buildGameStats(sumTenSessions, sumTenPayments)},
            ];
            ...
            setData({
                ...
                sumTen: buildGameStats(sumTenSessions, sumTenPayments),
            });
        }).catch(error => {
...
export function AdminView({initialTab}: { initialTab?: AdminTab } = {}) {
    ...
    useEffect(() => {
        ...
        const sessionReads = [
            ...
            {name: "SumTen", collectionName: "sumTenSessions"},
        ];
        const reads = [
            {label: "players", promise: getDocs(collection(db, "players"))},
            {label: "payments", promise: getDocs(collection(db, "payments"))},
        ...
        <div className="adm-root">
            ...
            <div className={`adm-drawer${drawerOpen ? " open" : ""}`}>
                ...
                <div className="adm-drawer-nav">
                   ...
                   {TABS.filter(t => ["kcse", "biblequiz", "mathquiz", "bioquiz", "genquiz", "sudoku", "connectdots", "sumten", "streetbongo"].includes(t.id)).map(t => {
                       const Icon = t.icon;
                       return (
                           <button key={t.id} className={`adm-tab${tab === t.id ? " active" : ""}`}
                                   onClick={() => changeTab(t.id)}>
