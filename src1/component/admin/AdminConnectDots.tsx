// AdminConnectDots.tsx — Connect Dots admin: payments, sessions, leaderboard
...
function ConnectDotsPayments() {
    ...
    useEffect(() => {
        Promise.all([
            getDocs(collection(db, "connectDotsPayments")),
            getDocs(query(collection(db, "payments"), where("game", "==", "CONNECT_DOTS"))),
        ]).then(([snap1, snap2]) => {
            const all = [
...
function ConnectDotsSessions() {
    ...
    useEffect(() => {
        getDocs(collection(db, "connectDotsSessions"))
        ...
        getDocs(collection(db, "dismissedConnectDotsPayments"))
        ...
        <div style={s.card}>
            ...
            {tab === "stuck" ? (<>
                <div style={{ background: "#fff1f2", border: "1px solid #fecdd3", borderRadius: 8, padding: "10px 14px", marginBottom: 14, color: "#9f1239", fontSize: "0.85rem" }}>
                    These players paid but never started a Connect Dots session. Click <strong>Grant Session</strong> to restore their access.
                </div>
                <div style={{ overflowX: "auto", borderRadius: 8, border: "1px solid #e8eaf0" }}>
                    <table style={s.table}>
                ...
                <div style={{ overflowX: "auto", borderRadius: 8, border: "1px solid #e8eaf0" }}>
                    <table style={s.table}>
                        <thead><tr>{["#", "Name", "Phone", "Score", "Level", "Stage", "Hints", "Mistakes", "Date"].map(h => <th key={h} style={s.th}>{h}</th>)}</tr></thead>
                        <tbody>
                            {filtered.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE).map((r, i) => (
                                <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#fafafe" }}>
                                    ...
                                    <td style={s.td}>{r.mistakes ?? 0}</td>
                                    <td style={s.td}>{r.playedAt?.toDate?.()?.toLocaleString('en-GB') ?? "—"}</td>
                                </tr>
                            ))}
                            {!filtered.length && <tr><td colSpan={9} style={{ ...s.td, textAlign: "center", color: "#aaa" }}>No sessions</td></tr>}
                        </tbody>
                    </table>
                </div>
                <Pagination total={filtered.length} page={page} setPage={setPage} />
...
// ── Leaderboard Tab ───────────────────────────────────────────────────────────
function ConnectDotsLeaderboard() {
    const [rows, setRows] = useState<any[]>([]);
    const [page, setPage] = useState(0);

    useEffect(() => {
        getDocs(collection(db, "connectDotsLeaderboard"))
...
// ── Main Export ───────────────────────────────────────────────────────────────
type ConnectDotsTab = "payments" | "sessions" | "leaderboard";

export function AdminConnectDots() {
    const [tab, setTab] = useState<ConnectDotsTab>("payments");

    const tabs: { id: ConnectDotsTab; label: string }[] = [
    ...
    return (
        <div>
            ...
            {tab === "payments"    && <ConnectDotsPayments />}
            {tab === "sessions"    && <ConnectDotsSessions />}
            {tab === "leaderboard" && <ConnectDotsLeaderboard />}
        </div>
    );
}
