...
function BQQuestions() {
    ...
    useEffect(() => {
        getDocs(collection(db, "mathQuizQuestions"))
    ...
    const save = async (q: BQQuestion) => {
        if (q.id) {
            ...
            await import("firebase/firestore").then(({ updateDoc, doc: fDoc }) =>
                updateDoc(fDoc(db, "mathQuizQuestions", id!), data));
            setQuestions(prev => prev.map(x => x.id === id ? q : x));
        } else {
            const ref = await addDoc(collection(db, "mathQuizQuestions"), q);
    ...
    const del = async (id: string) => {
        await deleteDoc(doc(db, "mathQuizQuestions", id));
    ...
    const deleteAllQuestions = async () => {
        ...
        questions.forEach(q => batch.delete(doc(db, "mathQuizQuestions", q.id!)));
        await batch.commit();
        setQuestions([]);
    ...
    const deleteDuplicateQuestions = async () => {
        ...
        for (let i = 0; i < idsToDelete.length; i += 450) {
            const batch = writeBatch(db);
            idsToDelete.slice(i, i + 450).forEach(id => batch.delete(doc(db, "mathQuizQuestions", id)));
            await batch.commit();
        }
    ...
    const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const batch = writeBatch(db);
            parsed.forEach(q => batch.set(doc(collection(db, "mathQuizQuestions")), q));
            await batch.commit();
            const snap = await getDocs(collection(db, "mathQuizQuestions"));
...
function usePage<T>(items: T[]) {
    ...
    const total = Math.max(1, Math.ceil(items.length / PAGE_S));
    const safe  = Math.min(page, total);
    const slice = items.slice((safe - 1) * PAGE_S, safe * PAGE_S);
    return { page: safe, setPage, total, slice, from: items.length ? (safe - 1) * PAGE_S + 1 : 0, to: Math.min(safe * PAGE_S, items.length) };
}
...
function BQSessions() {
    ...
    useEffect(() => {
        ...
        getDocs(query(collection(db, "payments"), where("game", "==", "MATHQUIZ")))
            ...
            .catch(() => {});
        getDocs(collection(db, "dismissedMathPayments"))
    ...
    const grantSession = async (p: any) => {
        ...
        setGranting(phone07);
        try {
            await setDoc(doc(db, "grantedMathSessions", phone07), {
                                ...
                                <tr key={p._id}>
                                    ...
                                    <td style={s.td}>
                                        <div style={{ display: "flex", gap: 6 }}>
                                            ...
                                            <button onClick={() => {
                                                setDoc(doc(db, "dismissedMathPayments", p._id), { dismissedAt: new Date() }).catch(() => {});
                        ...
                        <tbody>
                            {slice.map((r, i) => (
                                <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#fafafe" }}>
                                    <td style={s.td}>{(page-1)*PAGE_S + i + 1}</td>
                                    <td style={s.td}>{r.name ?? "—"}</td>
                                    <td style={s.td}>{r.phone ?? "—"}</td>
...
function BQLeaderboard() {
    ...
    useEffect(() => {
        getDocs(collection(db, "mathQuizLeaderboard"))
            .then(snap => setRows(snap.docs.map(d => ({ id: d.id, ...d.data() }))
