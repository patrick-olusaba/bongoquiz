// AdminSumTen.tsx — SumTen admin: payments, sessions, leaderboard
...
function SumTenPayments() {
    ...
    useEffect(() => {
        Promise.all([
            getDocs(collection(db, "sumTenPayments")),
            getDocs(query(collection(db, "payments"), where("game", "==", "SUMTEN"))),
        ]).then(([snap1, snap2]) => {
            const all = [
...
function SumTenSessions() {
    ...
    useEffect(() => {
        getDocs(collection(db, "sumTenSessions"))
        ...
        Promise.all([
            getDocs(collection(db, "sumTenPayments")),
            getDocs(query(collection(db, "payments"), where("game", "==", "SUMTEN"))),
        ]).then(([snap1, snap2]) => setPayments([
            ...snap1.docs.map(d => ({ _id: d.id, ...d.data() })),
            ...snap2.docs.map(d => ({ _id: d.id, ...d.data() })),
        ])).catch(() => {});
        getDocs(collection(db, "dismissedSumTenPayments"))
    ...
    const grantSession = async (p: any) => {
        ...
        setGranting(phone07);
        try {
            await setDoc(doc(db, "grantedSumTenSessions", phone07), {
                phone: phone07, name: p.name ?? "", grantedAt: new Date(), grantedBy: "admin", paymentId: p._id,
            });
            writeAdminAudit({
                action: "Granted SumTen session",
                target: phone07,
                game: "SumTen",
        ...
        <div style={s.card}>
            ...
            {tab === "stuck" ? (<>
                <div style={{ background: "#fff1f2", border: "1px solid #fecdd3", borderRadius: 8, padding: "10px 14px", marginBottom: 14, color: "#9f1239", fontSize: "0.85rem" }}>
                    These players paid but never started a SumTen session. Click <strong>Grant Session</strong> to restore their access.
                </div>
                <div style={{ overflowX: "auto", borderRadius: 8, border: "1px solid #e8eaf0" }}>
                    <table style={s.table}>
                                ...
                                <tr key={p._id}>
                                    ...
                                    <td style={s.td}>
                                        <div style={{ display: "flex", gap: 6 }}>
                                            ...
                                            <button onClick={() => {
                                                setDoc(doc(db, "dismissedSumTenPayments", p._id), { dismissedAt: new Date() }).catch(() => {});
                                                writeAdminAudit({
                                                    action: "Dismissed stuck SumTen payment",
                                                    target: p._id,
                                                    game: "SumTen",
                ...
                <div style={{ overflowX: "auto", borderRadius: 8, border: "1px solid #e8eaf0" }}>
                    <table style={s.table}>
                        <thead><tr>{["#", "Name", "Phone", "Score", "Level", "Game #", "Completed", "Hints", "Reason", "Date"].map(h => <th key={h} style={s.th}>{h}</th>)}</tr></thead>
                        <tbody>
                            {filtered.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE).map((r, i) => (
                                <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#fafafe" }}>
                                    ...
                                    <td style={s.td}>{r.gameInLevel ?? "—"}</td>
                                    <td style={s.td}>{r.totalGamesCompleted ?? 0}</td>
                                    <td style={s.td}>{r.hintsUsed ?? 0}</td>
                                    <td style={s.td}>{r.endedReason ?? "—"}</td>
                                    <td style={s.td}>{r.playedAt?.toDate?.()?.toLocaleString('en-GB') ?? "—"}</td>
                                </tr>
                            ))}
                            {!filtered.length && <tr><td colSpan={10} style={{ ...s.td, textAlign: "center", color: "#aaa" }}>No sessions</td></tr>}
                        </tbody>
                    </table>
                </div>
                <Pagination total={filtered.length} page={page} setPage={setPage} />
...
// ── Leaderboard Tab ───────────────────────────────────────────────────────────
function SumTenLeaderboard() {
    const [rows, setRows] = useState<any[]>([]);
    const [page, setPage] = useState(0);

    useEffect(() => {
        getDocs(collection(db, "sumTenLeaderboard"))
...
// ── Main Export ───────────────────────────────────────────────────────────────
type SumTenTab = "payments" | "sessions" | "leaderboard";

export function AdminSumTen() {
    const [tab, setTab] = useState<SumTenTab>("payments");

    const tabs: { id: SumTenTab; label: string }[] = [
    ...
    return (
        <div>
            ...
            {tab === "payments"    && <SumTenPayments />}
            {tab === "sessions"    && <SumTenSessions />}
            {tab === "leaderboard" && <SumTenLeaderboard />}
        </div>
    );
}
