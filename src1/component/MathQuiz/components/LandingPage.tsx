...
import { Timer, Trophy, Star, X, User } from 'lucide-react';
...
export const LandingPage: React.FC<Props> = ({ onStartGame, playerName, setPlayerName, playerPhone, setPlayerPhone, leaderboard }) => {
  ...
  const handleShare = () => {
    const text = `🧮 Play Math Bongo Quiz — test your math skills and win!\n${window.location.href}`;
    if (navigator.share) navigator.share({ title: 'Math Bongo Quiz', text, url: window.location.href }).catch(() => {});
  ...
  return (
    <div onClick={addRipple} style={{ position: 'relative', zIndex: 1, minHeight: '100dvh', height: '100dvh', maxHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: "'Trebuchet MS','Segoe UI',sans-serif", background: '#060412', color: '#fff', overflowX: 'hidden', overflowY: 'auto', WebkitOverflowScrolling: 'touch', paddingBottom: 84, boxSizing: 'border-box' }}>
      ...
      {/* Main content */}
      <div style={{ position: 'relative', zIndex: 2, width: '100%', maxWidth: 600, padding: '72px 1.25rem 0', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
