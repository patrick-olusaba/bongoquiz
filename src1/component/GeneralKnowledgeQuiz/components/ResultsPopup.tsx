...
const ResultsPopup: FC<ResultsPopupProps> = ({ player, onPlayAgain, onMenu }) => {
    ...
    useEffect(() => {
        ...
        if (player.score > prev) {
            setIsNewBest(true);
            localStorage.setItem('quiz_best_score', String(player.score));
        }
        ...
        const name  = localStorage.getItem('bongo_player_name')  ?? player.name ?? 'Player';
        if (/^07\d{8}$/.test(phone)) {
            httpsCallable(getFunctions(), 'saveGenQuizSession')({
                name, phone,
                score:   player.score,
    ...
    const rating =
        player.score >= 1000 ? '🌟 Legendary! Quiz Master!'
