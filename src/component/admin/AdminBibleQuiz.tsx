...
function BQQuestions() {
    ...
    useEffect(() => {
        getDocs(collection(db, "bibleQuizQuestions"))
    ...
    const save = async (q: BQQuestion) => {
        if (q.id) {
            ...
            await import("firebase/firestore").then(({ updateDoc, doc: fDoc }) =>
                updateDoc(fDoc(db, "bibleQuizQuestions", id!), data));
            setQuestions(prev => prev.map(x => x.id === id ? q : x));
        } else {
            const ref = await addDoc(collection(db, "bibleQuizQuestions"), q);
    ...
    const del = async (id: string) => {
        await deleteDoc(doc(db, "bibleQuizQuestions", id));
    ...
    const deleteAllQuestions = async () => {
        ...
        questions.forEach(q => batch.delete(doc(db, "bibleQuizQuestions", q.id!)));
        await batch.commit();
        setQuestions([]);
    ...
    const deleteDuplicateQuestions = async () => {
        ...
        for (let i = 0; i < idsToDelete.length; i += 450) {
            const batch = writeBatch(db);
            idsToDelete.slice(i, i + 450).forEach(id => batch.delete(doc(db, "bibleQuizQuestions", id)));
            await batch.commit();
        }
    ...
    const exportToCSV = () => {
        ...
        a.href = url;
        a.download = `bible_quiz_questions_${new Date().toISOString().split('T')[0]}.csv`;
        a.click();
    ...
    const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const batch = writeBatch(db);
            parsed.forEach(q => batch.set(doc(collection(db, "bibleQuizQuestions")), q));
            await batch.commit();
            const snap = await getDocs(collection(db, "bibleQuizQuestions"));
...
export function AdminBibleQuiz() {
