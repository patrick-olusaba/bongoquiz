...
export const PaymentScreen: React.FC<Props> = ({ playerName, playerPhone, onPayAndPlay, onCancel }) => {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const res = await fetch('https://us-central1-bongoquiz-23ad4.cloudfunctions.net/deposit', {
        ...
        body: JSON.stringify({ name: playerName, phone: phone254, amount: 20, trigger: 'R1R2', game: 'MATHQUIZ' }),
      }).then(r => r.json());
  ...
  const statusMsg = timedOut ? "Still waiting — you'll enter once confirmed."
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ background: 'linear-gradient(145deg,#001a08,#000d04)', border: '1px solid rgba(0,220,100,0.2)', borderRadius: 24, padding: '2rem', width: '100%', maxWidth: 380, textAlign: 'center', boxShadow: '0 25px 50px rgba(0,0,0,0.6)' }}>
        ...
        <h2 style={{ color: '#fff', fontSize: '1.4rem', fontWeight: 900, margin: '0 0 0.25rem' }}>Math Quiz</h2>
