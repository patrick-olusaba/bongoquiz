...
const MathGameIntro: FC<Props> = ({ onDone }) => {
  ...
  return (
    <div style={{
      ...
      background: 'radial-gradient(ellipse at 50% 40%, #002a10 0%, #001208 55%, #000 100%)',
      ...
      {showLogo && [0, 180, 360].map(delay => (
        <div key={delay} style={{
          ...
          animation: 'mathShockRing 1s ease-out forwards',
          animationDelay: `${delay}ms`,
        }} />
      ))}

      {/* Orbiting math symbols */}
      {showLogo && ['+','−','×','÷','π','√'].map((sym, i) => (
        <div key={i} style={{
          position: 'absolute', fontSize: i % 2 === 0 ? '1.2rem' : '0.9rem', fontWeight: 900,
          color: i % 2 === 0 ? 'rgba(0,220,100,0.9)' : 'rgba(74,222,128,0.6)',
          textShadow: '0 0 10px rgba(0,220,100,0.8)',
          animation: 'mathOrbit 1.6s ease-in-out infinite',
          animationDelay: `${i * 0.25}s`,
          transformOrigin: `${70 + i * 10}px center`,
        }}>{sym}</div>
      ))}
      ...
      {showSub && (
        <div style={{ marginTop: 32, width: 'min(240px,60vw)', height: 3, background: 'rgba(255,255,255,0.1)', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{ height: '100%', borderRadius: 4, background: 'linear-gradient(90deg,#00DC64,#fff)', animation: 'mathBarFill 1.6s linear forwards' }} />
        </div>
      )}

      <style>{`
        @keyframes mathShockRing { from{transform:scale(1);opacity:0.8} to{transform:scale(40);opacity:0} }
        @keyframes mathBarFill   { from{width:0%} to{width:100%} }
        @keyframes mathOrbit     { 0%{transform:rotate(0deg) translateX(var(--r,70px)) rotate(0deg);opacity:0.9} 50%{opacity:0.4} 100%{transform:rotate(360deg) translateX(var(--r,70px)) rotate(-360deg);opacity:0.9} }
      `}</style>
    </div>
  );
};
