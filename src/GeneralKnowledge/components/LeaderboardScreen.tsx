...
const LeaderboardScreen: FC<Props> = ({ playerScore, playerName = "You", onPlayAgain, onClose }) => {
  ...
  const entries: LeaderboardEntry[] = (() => {
    const saved: { name: string; score: number }[] = JSON.parse(localStorage.getItem('quiz_leaderboard') ?? '[]');
    const merged = saved.map((e, i) => ({
      rank: i + 1, name: e.name, score: e.score,
      isCurrentPlayer: e.name === playerName && e.score === playerScore,
    }));
    if (playerScore > 0 && !merged.some(e => e.isCurrentPlayer)) {
      merged.push({ rank: 0, name: playerName, score: playerScore, isCurrentPlayer: true });
    ...
    <div className={`lb-root${visible ? " lb-root--visible" : ""}`}>
      <div className="lb-panel">
        ...
        <div className="lb-header">
          ...
          <p className="lb-subtitle">Top General Knowledge players</p>
